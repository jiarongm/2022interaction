...
void draw(){
  ...
  if(moving){
    fill(0,200,0,128);///綠色半透明 標示正要移動的棋子
    ellipse(50+25+moveJ*50,50+25+moveI*50,40,40);//原來的位置
    drawChess(mouseX,mouseY,moveID);///正在移動的棋子(隨著滑鼠動)
  }
}
...
void drawChess(int x, int y, int id){
    if(id==0)return;//沒有旗子就不用進來畫棋
    fill(255);
    ellipse( x, y, 40, 40);
    /*if(id==9){
      fill(0,200,0);//綠色標記要移動的棋子
      ellipse( x, y, 40, 40);
    }
    else*/ 
    if(id>0){//黑
