...
class Ball {
  ...
  void collision() {
    loadPixels();
    int i=int( (y+21.5)*500+x);
    if (x>0&&y>0&&x<490&&y<590) {
      color pcolor=pixels[(y+21.5)*500+x].color();
      if (pcolor.blue()>pcolor.red()) {
        vx=0;
        vy=0;
      } else {
        x+=vx;
      ...
      println(pixels[51*500+201]==color(255, 229, 229));
      println("**", x, "--ball--", y, "**");
